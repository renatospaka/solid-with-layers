...
export default class TransactionDatabaseRepository implements TransactionRepository {
  ...
  async save(transaction: Transaction): Promise<void> {
    await this.connection.query("insert into app.transaction(code, amount, number_installments, payment_method) values ($1, $2, $3, $4)", [transaction.code, transaction.amount, transaction.numberInstallments, transaction.paymentMethod]);
    for (const installment of transaction.installments) {
      await this.connection.query("insert into app.installment (code, number, amount) values ($1, $2, $3)", [transaction.code, installment.number, installment.amount]);
    }
  }

  async get(code: string): Promise<Transaction> {
    const transactionData = await this.connection.one("select * from app.transaction where code = $1", [code]);
    ...
    const installmentsData = await this.connection.query("select * from app.installment where code = $1", [code]);
